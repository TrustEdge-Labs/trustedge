...
use trustedge_pubky::mock::{mock_send_trusted_data, MockPubkyAdapter};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    rt.block_on(async {
        ...
        println!(
            ...
            ((large_envelope.len() - large_data.len()) as f64 / large_envelope.len() as f64)
                * 100.0
        );
        println!();
...
mod tests {
    ...
    async fn test_exact_api_specification() {
        ...
        let envelope = send_trusted_data(data, &bob_pubky_id, storage)
            .await
