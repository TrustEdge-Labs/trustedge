...
pub fn send_trusted_data(
    ...
    let backend = MockPubkyBackend::with_shared_storage(storage.clone());
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let decrypted_data = receive_trusted_data(&sealed_envelope, &bob_keypair.private)?;
    ...
    println!(
        "   Data integrity: {}",
        secret_data == decrypted_data.as_slice()
    );
    println!();
    ...
    let large_envelope = send_trusted_data(&large_data, &bob_pubky_id, storage)?;
...
mod tests {
    ...
    async fn test_exact_api_specification() {
        ...
        let envelope =
            send_trusted_data(data, &bob_pubky_id, storage).expect("send_trusted_data failed");
    ...
    async fn test_api_with_different_algorithms() {
        ...
        // Test with Ed25519
        let ed25519_adapter = MockPubkyBackend::with_shared_storage(storage.clone());
        let ed25519_keypair = KeyPair::generate(AsymmetricAlgorithm::Ed25519)
            .expect("Failed to generate Ed25519 key");
        let ed25519_pubky_id = ed25519_adapter
            .publish_public_key(&ed25519_keypair.public)
        ...
        let resolved_key = ed25519_adapter
            .resolve_public_key(&ed25519_pubky_id)
