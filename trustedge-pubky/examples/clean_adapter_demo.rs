...
use trustedge_pubky::{
    create_pubky_adapter_random, send_trusted_data, receive_trusted_data, PubkyAdapter
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Original message: {:?}", std::str::from_utf8(secret_message).unwrap());
    ...
    println!("   Overhead: {} bytes ({:.1}%)", 
        ...
        ((sealed_envelope.len() - secret_message.len()) as f64 / secret_message.len() as f64) * 100.0
    );
    println!();
    ...
    println!("   Keys match: {}", resolved_bob_key.id() == bob_keypair.public.id());
    println!();
    ...
    let alice_ed25519_pubky_id = alice_adapter.publish_public_key(&alice_ed25519.public).await?;
    
    println!("✅ Ed25519 key published: {}", alice_ed25519_pubky_id);
    
    let resolved_ed25519 = bob_adapter.resolve_public_key(&alice_ed25519_pubky_id).await?;
...
mod tests {
    ...
    async fn test_clean_adapter_api() {
        ...
        let envelope = send_trusted_data(message, &bob_pubky_id, &alice_adapter).await
