...
fn generate_keypair(output: Option<PathBuf>, seed: Option<String>, id_only: bool) -> Result<()> {
    let backend = if let Some(seed_hex) = seed {
        ...
        create_pubky_backend_random().context("❌ Failed to create random Pubky backend")?
    };
    ...
    if let Some(output_path) = output {
        ...
        std::fs::write(&output_path, &private_key_hex).with_context(|| {
            format!(
...
fn resolve_key(pubky_id: String, output: Option<PathBuf>, info: bool) -> Result<()> {
    ...
    let backend = create_pubky_backend_random().context("Failed to create Pubky backend")?;

    let public_key = backend
        .resolve_public_key_sync(&pubky_id)
...
fn decrypt_data(input: PathBuf, output: PathBuf, key: PathBuf) -> Result<()> {
    // Read envelope
    let envelope = std::fs::read(&input).context("Failed to read envelope file")?;

    // Read private key
    let key_hex = std::fs::read_to_string(&key).context("Failed to read private key file")?;
    ...
    let decrypted_data =
        receive_trusted_data(&envelope, &private_key).context("Failed to decrypt envelope")?;
