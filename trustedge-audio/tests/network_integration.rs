...
async fn wait_for_server_ready(addr: SocketAddr, timeout_secs: u64) -> Result<()> {
    ...
    Err(anyhow::anyhow!("Server not ready after {} seconds", timeout_secs))
}
...
async fn test_data_integrity() -> Result<()> {
    ...
    let test_data = b"Data integrity test content with specific patterns: 0123456789ABCDEF".repeat(100);
    let mut test_file = NamedTempFile::new()?;
    test_file.write_all(&test_data)?;
    let test_file_path = test_file.path();
    ...
    assert!(
        ...
        output_files.iter().map(|e| e.file_name()).collect::<Vec<_>>()
    );
...
async fn test_large_file_transfer() -> Result<()> {
    ...
    let client_output = Command::new(&client_binary)
        .args([
            "--server", &server_addr.to_string(),
            "--file", test_file_path.to_str().unwrap(),
            "--key-hex", key_hex,
            "--chunk-size", "1024", // Use small chunks to test chunking
            "--verbose",
        ])
        .output()?;
