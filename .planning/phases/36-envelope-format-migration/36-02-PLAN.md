...
<task type="auto">
  ...
  **1. Create `decrypt_chunk_v2()` method:**
  - Signature: `fn decrypt_chunk_v2(&self, chunk: &NetworkChunk, decryption_key: &SigningKey, encryption_key: &[u8; 32], nonce_prefix: &[u8; 8], is_last_chunk: bool) -> Result<Vec<u8>>`
  ...
  **3. Rewrite `unseal()` with try-then-fallback:**
  - After verification passes, attempt v2 decryption first (expected common case):
    ...
    - For each chunk: call `decrypt_chunk_v2(chunk, decryption_key, &encryption_key, &nonce_prefix, is_last)` where `is_last = (i == sorted_chunks.len() - 1)`
...
<task type="auto">
  ...
  **4. `test_v2_deterministic_nonces_are_unique()`:**
  ...
  <verify>
    ...
    <manual>12+ envelope tests pass (8 existing + 4 new); v2 multi-chunk roundtrip demonstrates correct HKDF-once + deterministic nonce behavior</manual>
  </verify>
  <done>4 new tests added: v2 multi-chunk roundtrip, v2 single-chunk roundtrip, version field serialization, deterministic nonce uniqueness. All envelope tests pass. TST-01 (no regression) and TST-02 (multi-chunk v2 end-to-end) requirements met</done>
</task>
...
5. `cargo clippy -p trustedge-core -- -D warnings` passes
</verification>
...
- VER-02 requirement fully addressed
