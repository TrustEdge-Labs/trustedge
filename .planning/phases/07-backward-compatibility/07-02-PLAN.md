...
7. **Troubleshooting**
   - "Cannot find type in this scope" → check Cargo.toml has trustedge-core
   - "Multiple versions of trustedge-core" → run `cargo tree`, update deps
   - Shell crates fail → should not happen, report as bug
...
Place this BEFORE the existing "🏗️ Architecture Improvements" section in [Unreleased].
