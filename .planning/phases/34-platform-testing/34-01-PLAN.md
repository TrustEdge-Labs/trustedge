...
must_haves:
  truths:
    ...
    - "Config::from_env uses sensible defaults (all vars optional in verify-only mode) and falls back to default port when PORT is unparseable"
...
Output: `crates/platform-server/tests/wiring.rs` with 5 tests, updated Cargo.toml dev-dependencies.
</objective>
...
2. Create `crates/platform-server/tests/wiring.rs` with the MPL-2.0 copyright header.
   ...
   c. `test_config_from_env_invalid_port_uses_default`: Set `PORT=not_a_number` env var, call `Config::from_env()`. The implementation uses `.parse().unwrap_or(3001)`, so an invalid port value falls back to 3001 rather than returning an error. Assert that `Config::from_env()` succeeds and port equals 3001.

      NOTE on "missing required env vars" (user decision): After inspecting Config::from_env(), ALL env vars have defaults — `PORT` defaults to "3001" (with parse fallback), `JWT_AUDIENCE` defaults to "trustedge-platform", `DATABASE_URL` defaults to a localhost connection string (postgres-only). In verify-only mode (no postgres), there are zero required env vars. This test validates the most likely error path: a malformed value rather than a missing value. The success criteria below reflect this finding.

   d. `test_appstate_construction_and_router_health`: Construct a `KeyManager::new()`, create `AppState { keys: Arc::new(RwLock::new(key_manager)) }`, call `create_router(state)`, send a GET /healthz request via `tower::ServiceExt::oneshot`, assert HTTP 200 and body contains `"status":"OK"`.

   e. `test_router_verify_rejects_empty_body`: Using the same router construction as above, send a POST /v1/verify with empty JSON `{}`, assert HTTP 400 (bad request due to missing fields — serde deserialization fails).
...
All 5 tests should pass. Also verify: `cargo clippy -p trustedge-platform-server --no-default-features --tests -- -D warnings` has no warnings.
  </verify>
  <done>
`cargo test -p trustedge-platform-server --no-default-features --test wiring` runs 5 tests, all pass. AppState constructs, router responds to health, Config defaults work, invalid port falls back gracefully.
  </done>
</task>
...
<success_criteria>
- `cargo test -p trustedge-platform-server --no-default-features --test wiring` passes with 5 tests
- Tests validate Config defaults, custom env vars, invalid port fallback, AppState+router health, and verify endpoint rejection
- Config::from_env() has no required env vars in verify-only mode (all have defaults) — invalid-value fallback path tested instead of missing-var error path
- No clippy warnings
