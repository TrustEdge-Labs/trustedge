...
must_haves:
  truths:
    ...
    - "Duplicate ManifestError eliminated - only ManifestFormatError exists in trst-protocols, aliased as ManifestError in core for backward compat"
...
Note on INTG-01: Roadmap requirement INTG-01 is satisfied through core's re-exports — `trustedge_core::CamVideoManifest` etc. continue to work, sourced from trst-protocols per CONTEXT.md user decisions. The re-export layer in core's lib.rs ensures all existing consumers see no API change.
