...
must_haves:
  truths:
    - "All 17 verify-core tests pass in the consolidated crate (5 unit in engine.rs, 5 unit in validation.rs, 7 integration)"
...
Purpose: Validate that the consolidated crate passes all 28 original tests (17 from verify-core + 11 from platform-api), update CI to treat trustedge-platform as Tier 1 blocking, and update workspace documentation.
...
<task type="auto">
  ...
  <verify>
    ...
    `cargo test -p trustedge-platform --test verify_integration --features http` passes all 7 tests.
    `cargo test -p trustedge-platform --lib` passes all ~10 unit tests.
  ...
  <done>
    All 17 verify-core tests pass (10 unit + 7 integration). 11 platform-api tests present as #[ignore]. CI validates trustedge-platform as Tier 1. CLAUDE.md and DEPENDENCIES.md updated. Total: 28 tests accounted for (17 running, 11 ignored/DB-dependent).
  </done>
</task>
...
<verification>
1. `cargo test -p trustedge-platform --lib` — 10 unit tests pass
...
<success_criteria>
- 17 verify-core tests pass: 5 engine.rs unit + 5 validation.rs unit + 7 integration
- 11 platform-api tests present with #[ignore] (require PostgreSQL)
- Total 28 tests accounted for per SVC-04
