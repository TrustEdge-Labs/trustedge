...
**Stats:** 21 files changed, 2,211 insertions, 141 deletions, 19 commits
...
- Eliminated panic paths: `beneficiary()`/`issuer()` return `Result<VerifyingKey>` instead of `expect()`
- Enforced 0600 Unix permissions on secret key files; stderr warning on non-Unix
- Guarded nonce construction against chunk index overflow at 2^24 boundary in both seal/unseal paths
