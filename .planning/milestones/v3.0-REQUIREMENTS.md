...
# Requirements Archive: v3.0 Release Polish
...
- [x] **CORE-01**: `generate_aad()` uses `.expect("AAD serialization is infallible")` instead of `.unwrap()`
- [x] **CORE-02**: `Envelope::hash()` returns `Result` instead of `unwrap_or_default()` that silently produces empty-input hash on failure
