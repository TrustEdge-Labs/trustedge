...
- **v1.7 Security & Quality Hardening** - Phases 31-34 (active)
...
### v1.7 Security & Quality Hardening
...
- [x] 32-02-PLAN.md — Isolate pubky crates into experimental workspace and clean root deps
- [x] 32-03-PLAN.md — Rewrite CI scripts and documentation to reflect new workspace structure
...
Plans:
- [x] 33-01-PLAN.md — Deduplicate verify handler validation and receipt construction
- [x] 33-02-PLAN.md — Harden CORS policy and document CA module as library-only
...
Plans:
- [x] 34-01-PLAN.md — Platform-server wiring tests (Config, AppState, router health)
- [x] 34-02-PLAN.md — Router builder fidelity, CORS parity test, full verify round-trip
...
- **Insertions:** +6,082
- **Deletions:** -1,855
- **Timeline:** 2026-02-22 to 2026-02-23

## Key Accomplishments

1. **Secret<T> wrapper with zeroize** — Custom in-house Secret<T> type with Zeroize/ZeroizeOnDrop, redacted Debug, no Display/Deref/Serialize. Applied to all sensitive fields across YubiKeyConfig, SoftwareHsmConfig, LoginRequest, CAConfig, AuthService. CI Step 23 enforces no regression.
2. **Deprecated facade crates deleted** — trustedge-receipts and trustedge-attestation removed from workspace. Experimental pubky crates isolated into separate standalone workspace at crates/experimental/.
3. **Verify handler deduplicated** — validate_verify_request_full() extracts 4 inline validation checks into single always-compiled function. build_receipt_if_requested() shares receipt construction with manifest_digest_fn parameter to avoid feature-flag coupling.
4. **CORS hardened** — Non-postgres build uses CorsLayer::new() (same-origin only). Postgres build restricts headers to Content-Type, Authorization, Accept. CA module documented as library-only with Axum coupling removed.
5. **Platform-server wiring tests** — 5 integration tests validate Config defaults, custom env vars, invalid port fallback, AppState construction, and verify endpoint rejection. OnceLock<Mutex> serializes env-var tests.
6. **Full HTTP verify round-trip** — 4 new integration tests: CORS parity between two routers, sign-then-verify pipeline with receipt, JWS receipt verified against JWKS Ed25519 key, wrong-key signature failure. Total: 11 verify integration tests.

---
*Archived: 2026-02-23*
*Last updated: 2026-02-23 after milestone completion*
