...
- [x] **CLI-01**: User can run `trst attest-sbom` to bind a CycloneDX JSON SBOM to a binary artifact, producing a `.te-attestation.json` file
- [x] **CLI-02**: User can run `trst verify-attestation` to verify an attestation document locally, with optional binary/SBOM hash checking
- [x] **CLI-03**: CLI rejects 0-byte binaries, non-JSON SBOMs, and binaries >256MB with clear error messages
